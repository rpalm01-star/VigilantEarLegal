...
惡劣天氣預警來自**涵蓋 88 個國家和地區的 53 個官方政府來源**——所有使用者均可免費使用。紐西蘭的民防緊急警報也包含在內，並有獨立的「**緊急警報**」開關。授權條款要求註明出處的來源，列在本頁末尾的「資料來源」中。應用只會選用覆蓋你所在位置的資料來源。在日本，應用還會讀取 JMA 的**預先通報**——即在臺風或暴雨來臨前數天釋出的通俗易懂的說明，而不僅僅是危險已經到來時才釋出的警報——以及突發強降雨和山體滑坡通報。預先通知會安靜地顯示，不會使用專為真正警報保留的聲音和振動。
...
- **USGS 全球地震警報**——附近有地震報告時，你會感到一陣振動，並在地圖上看到有震感的區域。這是來自 USGS 官方資料來源的確認，而不是預警：如果你感到了晃動，它會告訴你那是什麼。裝置端的低沉轟鳴（次聲）感知可以在地面一動的瞬間就啟動核查。
...
graph LR
    ...
    W --> N["官方公共資料來源<br/>53 個氣象來源 · 88 個國家和地區<br/>紐西蘭民防 · USGS 地震"]
```
...
部分官方來源以要求註明出處的開放授權條款發布其預警：

- **墨西哥** — Servicio Meteorológico Nacional (CONAGUA), CC BY 4.0
- **阿根廷** — Servicio Meteorológico Nacional (Argentina), CC BY 4.0
- **德國** — Quelle: Länderübergreifendes Hochwasserportal (LHP), www.hochwasserzentralen.de, CC BY 4.0
- **紐西蘭** — National Emergency Management Agency (NEMA), CC BY 4.0
- **歐洲** — EUMETNET MeteoAlarm via MeteoGate, meteoalarm.org
