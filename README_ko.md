...
- **Apple Watch 컴패니언** — 손목에서 알림의 방향을 가리켜 어디를 봐야 할지 한눈에 알려줍니다. 앱 귀 아이콘, 위협 HUD 레이아웃, 그리고 두 번 탭하여 최소화할 수 있는 재설계된 Watch UI. Watch 앱이 열려 있지 않아도 알림에 방향 화살표가 계속 표시될 수 있습니다.
...
### 🪄 Practice Playground — 귀 없이 증명하기
**Playground Mode**는 모두에게 공개되어 있습니다: Home & Street 연습 (노크, 알람, 아기, 사이렌, 날씨), 다중 전화 및 대화 데모, 그리고 연습이 실제 이벤트인 척하지 않도록 명확한 워터마크가 있습니다. 패널을 닫으면 데모가 깨끗하게 종료됩니다 (GPS 스푸핑이 멈추거나 찌꺼기 플래그가 남지 않음).
...
- **데모 모드** — DEMO 워터마크가 있는 연습 알림 및 기능 미리보기.
...
**현재 Apple 마케팅 버전:** 1.0.7 (진행 중 / 배송 트랙). 최신 iOS (SpeechAnalyzer 시대)를 위해 빌드되었습니다.
