...
- **Benannte Stimmen (optional)** — Wenn Sie einem Sprecher einen Namen geben, bleiben dieser Name und ein kurzer Stimmabdruck im verschlüsselten Speicher dieses Telefons und **verlassen es nie**. Es gibt keine Cloud-Sicherung von Stimmen.
...
*   **Remote Link (optional — das Starten erfordert Power Pack+; das Beitreten ist kostenlos)**
    ...
    *   *Wenn keine direkte Verbindung möglich ist:* Können die beiden Telefone einander nicht direkt erreichen — unterschiedliche Netze, ein strenger Router —, werden die verschlüsselten Daten — Video, Untertitel und Text — über ein Relay weitergeleitet, das sie **nicht entschlüsseln kann**. Das Relay sieht durchaus, dass eine Verbindung besteht, welche Netzwerkadressen beteiligt sind und wie viele Daten fließen — wie jedes Relay es muss — und nichts weiter. Die App zeigt klar an, ob eine Verbindung **Direct** oder **Relayed** ist. Eine über ein Relay geführte Verbindung schließt sich nach einer Stunde selbst.
