...
- **Spracherkennung (auf dem Gerät)** — Wenn Untertitel aktiviert sind, transkribieren die Sprach-Frameworks Ihres Geräts Sprache in der Nähe in Text auf dem Telefon. Untertiteltext wird live angezeigt und von Vigilant Ear nicht als permanenter Transkriptverlauf archiviert; Debug-Protokolle enthalten keinen Untertitelinhalt. Damit Namen richtig geschrieben werden, kann die App diesem geräteinternen Erkenner zusätzlich eine kurze Liste von Wörtern übergeben, die bereits auf diesem Telefon vorhanden sind — die Anzeigenamen der von Ihnen verbundenen Constellation-Telefone sowie Titel und Interpret eines gerade von Shazam erkannten Songs. Sie liest **nicht** Ihre Kontakte, und diese Liste verlässt das Gerät nie.
- **Standort** — Wird verwendet, um erkannte Geräusche und Wetterwarngebiete auf der Karte zu platzieren, die Richtungsführung zu verbessern und sich zu merken, wie leise ein vertrauter Raum ist, damit die Musikerkennung das nicht bei jedem Öffnen der App neu lernen muss. Für diesen letzten Zweck wird nur ein Breitengrad gespeichert — kein Längengrad —, auf etwa 100 Meter gerundet, für höchstens acht Orte. Er bleibt in den eigenen Einstellungen der App auf diesem Telefon und wird niemals irgendwohin gesendet.
...
*   **Remote Link (optional — das Starten erfordert Power Pack+; das Beitreten ist kostenlos)**
    *   *Warum es das gibt:* Eine gehörlose oder schwerhörige Person kann kein Telefongespräch führen. Remote Link ist der Ersatz: Zwei Menschen sehen einander und schreiben — und können über das Video gebärden.
    *   *Was gesendet wird:* **Zu keinem Zeitpunkt Audio** — eine Remote-Link-Sitzung enthält überhaupt keine Audiospur. Live-Video und getippter Text laufen **direkt zwischen den beiden Telefonen**, wo die Netze es zulassen, Ende-zu-Ende verschlüsselt. Zum Aufbau hält unser Dienst kurz einen kurzlebigen Einladungscode zusammen mit den technischen Angaben, die beide Telefone brauchen, um einander zu finden. Dieses Postfach enthält **kein Video und keinen Text** und läuft binnen einer Stunde ab.
    *   *Untertitel:* Wenn Ihr Telefon zuhört, werden die erzeugten Untertitel als Text an das verbundene Telefon gesendet — in der Sprache, in der sie gehört wurden; das andere Telefon übersetzt sie auf dem Gerät in seine eigene Lesesprache. Das läuft über dieselbe verschlüsselte Verbindung wie das Video, niemals über unsere Server. **Pause** stoppt Video und Untertitel gleichzeitig, und Untertitel enden ganz, sobald Sie das Zuhören ausschalten.
    *   *Wenn keine direkte Verbindung möglich ist:* Das verschlüsselte Video und der Text werden über ein Relay geleitet, das sie **nicht entschlüsseln kann**. Das Relay sieht, dass eine Verbindung besteht, welche Netzwerkadressen beteiligt sind und wie viele Daten fließen — wie jedes Relay. Eine über ein Relay geführte Verbindung schließt sich nach einer Stunde selbst.
    *   *Nichts wird aufgezeichnet:* Kein Video, kein Ton und kein Text aus einem Remote Link wird auf einem der beiden Telefone gespeichert oder auf einem Server abgelegt.
    *   *Anbieter:* Wingdings (Einladungspostfach), Cloudflare (Relay — nur wenn keine direkte Verbindung möglich ist)
