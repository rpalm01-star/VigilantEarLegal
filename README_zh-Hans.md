...
这是一款专为聋人、重听及 CODA 群体打造的应用程序。大多数声音识别应用只会告诉你声音是*什么*。**Vigilant Ear 还能告诉你声音在哪里、是谁发出的，以及他们在说什么** —— 将 iPhone 变成一个实时的声学三录仪，描述你周围的声音。
...
- 👁️ **专为聋人/重听人士/CODA 打造。** 独特的触觉反馈、高对比度视觉效果、与颜色无关的提示、巨大的点击目标，并在整个过程中尊重“减弱动态效果 (Reduce Motion)”。

---

## 它的服务对象

- **想要了解声音环境的聋人、重听及 CODA 用户** —— 居家看护（敲门、闹钟、婴儿、电话）和街头看护（警笛、靠近），你可以一直开着并信任它。
...
### 🚨 识别重要声音 —— 并警告你
设备端分类器可识别数百种日常声音，并监控关键类别 —— **警笛、警报 —— 包括专门的汽车警报类别 ——、门铃/敲门声、婴儿哭声、附近的人和恶劣天气。** 当触发其中之一时，你会得到清晰的屏幕提醒、可选的**推送通知**以及独特的**触觉反馈** —— 即使应用在后台运行或手机处于睡眠状态。关键类别默认准备就绪，因此启用通知并不意味着“所有声音都提醒”。关闭所有提醒类别，引擎在后台会完全休眠以节省电池。**Sentinel** 层会将提醒与独立证据 —— 方向、运动和公共数据源 —— 交叉核对，因此触发的提醒是经过佐证的，而不是单个分类器的孤立猜测。
...
- **伙伴提醒直达手腕** —— 当已链接的 Constellation 伙伴的手机发出提醒时，它也可以送达你的 Watch，并附带方向。可靠性优化让手表端伴侣全天对 Watch 电池都很轻量。

### 💬 说话人模式 —— 实时、定向字幕 *(免费)*
开启**说话人模式 (Speaker Mode)**，Vigilant Ear 会将在你附近说话的人转录为**字幕块，每个声音一个。** 设备端的说话人日志记录 (diarization) 保持声音的区别 —— *谁* 在说 *什么* —— 并在内环上带有方向提示。实时说话人会高亮显示；随着空间需要，较旧的文本会滚动移出。字幕是免费的；自动翻译是可选的 Power Pack+ 层。字幕还可以**通过蓝牙听力设备朗读出来** —— 免费，在偏好设置中开启。
...
这是迄今最接近科幻中**万能翻译器**的产品——一台"直接听懂"的设备。Vigilant Ear 自行检测语言，跟踪房间里的每一位说话者，把所有人的话都以您的语言实时字幕呈现——无需耳机，无需设置，全程在设备上运行。
...
### 🎛️ 声学示波器 —— 像工程师一样看声音
以专业级视图实时呈现周围的声音：频谱、语谱图、1/3 倍频程 RTA 频段、色度与谐波分音——**所有人均可免费使用**。用于采集声音、训练自定义声音包的工具属于 Power Pack+。
...
使用两部或多部支持超宽带 (Ultra-Wideband) 的 iPhone（自 iPhone 11 以来的大多数机型），**Constellation** 会将它们配对，以便它们可以感知彼此的位置，并将每部手机听到的声音融合为更精确的声音来源图片 —— 一个分布式的无源监听阵列。 手机之间**直接连接** —— 无需路由器、无需同一 Wi-Fi 网络，也无需互联网；只要打开 Wi-Fi 即可，走的是 AirDrop 使用的同一条点对点链路。该功能仅限于拥有合适硬件的设备。早于对等设备连接时间的网格 (Mesh) 字幕不会被重传。
...
### ♿ 辅助功能优先
专为聋人/重听/CODA 和色盲用户打造：**颜色独立**的提示，**≥44 pt** 的点击目标，尊重**减弱动态效果 (Reduce Motion)**，多模式提醒（触觉 + 视觉 + Watch），以及一个启动验证屏幕，用清晰的绿色 / 灰色 / 红色（以及暗橙色的“不允许”）状态显示权限状态 —— 包括充当主提醒开关的通知授权。
...
- **实时字幕** —— 说话人模式，设备端，在硬件允许的情况下具有方向性，并可选择将字幕朗读到蓝牙听力设备。
- **常驻看护（Standing Watch）** —— 房间自身的状态，始终开启且无需任何设置：当房间保持其惯常模式时，是一盏稳定的青色指示灯；当有变化时 —— 新的声音、突然的安静或有什么正在靠近 —— 会变为琥珀色。
- **恶劣天气警报** —— 适用于您所在地区的 NWS、MeteoGate、CMA、KMA、JMA（日本）、ECCC（加拿大）、INMET（巴西）。
- **地震警报（USGS，全球）** —— 附近有地震报告时，您会感受到震动，并在地图上看到有震感的区域。来自 USGS 官方源的确认信息 —— 不是预警：如果您感到了晃动，它会告诉您那是什么。设备端的深层震感（次声）检测可以在地面移动的瞬间启动核实。
- **功能游乐场** —— 带有清晰 PREVIEW 水印的练习提醒和功能预览。
- **Apple Watch 辅助和实时活动** —— 一目了然的方向和最新提醒。
- **声学示波器** —— 专业级实时声音可视化，所有人均可免费使用（用于训练的采集工具属于 Power Pack+）。

**Power Pack+** 是一次性解锁（**不是订阅**），有 **90 天的免费试用期**。它增加了超能力：

- **说话人自动翻译** —— 将附近的语音在设备端翻译成你的语言。
- **Constellation** —— 通过超宽带共享听力的多部 iPhone，并支持伙伴消息。
- **音乐识别** —— ShazamKit 歌曲识别。
- **自定义声音包** —— 可自行训练的扩展分类器。
...
- **iPhone（完整体验）。** 竖屏横屏皆可 — 想怎么拿就怎么拿。需要立体声麦克风来进行测向。推荐使用 **iPhone 13 或更新机型**。
...
完全本地化 —— 界面、提醒和字幕 —— 支持**英语、西班牙语、葡萄牙语（巴西）、法语、德语、阿拉伯语、日语、简体中文、韩语、俄语和意大利语**（11 种语言）。遵循系统区域设置或在应用程序中手动选择。
