...
```mermaid
graph LR
    A["Doar ascultă"] --> B["Limba detectată automat"] --> C["Fiecare voce separată"] --> D["Toată camera subtitrată în limba ta"]
```
...
```mermaid
graph TD
    A["Tap microfon stereo"] --> B["Instantaneu buffer din pool"]
    B --> C["Clasificator de sunete<br/>(Apple Neural Engine)"]
    B --> Y["Clasificator de a doua opinie<br/>(YAMNet, ANE)"]
    C --> S["Sentinel — strat de dovezi<br/>coroborează, pune veto, escaladează"]
    Y --> S
    S --> H["Alerte · haptic · Watch · Live Activity"]
    B --> D["Matematică spațială<br/>TDOA · tendință de nivel → direcție · distanță · apropiere"]
    D --> R["Inel radar · hartă · cameră AR"]
    B --> F["Recunoașterea vorbirii<br/>(SpeechAnalyzer)"]
    B --> N["Limitele tururilor de vorbire<br/>(diarizator Sortformer, ANE)"]
    N --> E["Identitate vocală<br/>(embedding-uri ReDimNet, ANE)"]
    B --> E
    F --> G["Rânduri de subtitrare — câte unul pe voce"]
    E --> G
...
```mermaid
graph LR
    L["Apare subtitrarea în direct"] --> K["Audio-ul brut recitit<br/>cu contextul complet"] --> V["Comparație păzită<br/>(păstrează ce s-a auzit)"] --> W["Rând corectat liniștit<br/>≤ 2 s, apoi înghețat"]
```
...
```mermaid
graph LR
    P1["Vigilant Ear<br/>Pe dispozitivul tău"] --> W["Cache de alerte Wingdings<br/>o copie partajată · reîmprospătare la 5 minute"]
    W --> N["Fluxuri publice oficiale<br/>NWS · MeteoGate · JMA · KMA · CMA<br/>ECCC · BOM · INMET · NDMA · USGS"]
```
