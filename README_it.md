...
Le allerte di maltempo estremo arrivano da feed pubblici ufficiali — **NWS** (Stati Uniti), **MeteoGate** (Europa), **CMA** (Cina), **KMA** (Corea), **JMA** (Giappone), **ECCC** (Canada), **BOM** (Australia), **INMET** (Brasile), **NDMA** (India) e **SMN** (Messico) — gratis per tutti gli utenti. Le allerte del Messico sono pubblicate dal suo Servicio Meteorológico Nacional con licenza CC BY 4.0. I feed vengono limitati a quelli che coprono la zona in cui ti trovi. In Giappone l'app legge anche i **bollettini anticipati** della JMA — gli avvisi in linguaggio semplice emessi giorni prima di un tifone o di forti piogge, non solo le allerte diramate quando il pericolo è ormai arrivato — oltre ai bollettini su acquazzoni improvvisi e frane. Gli avvisi anticipati vengono mostrati in modo discreto, senza il suono e la vibrazione riservati a una vera allerta.
...
- **Allerte di maltempo estremo** — NWS, MeteoGate (Europa — servito sempre aggiornato dalla nostra cache delle allerte a 5 minuti), CMA, KMA, JMA (Giappone), ECCC (Canada), BOM (Australia), INMET (Brasile), NDMA (India) e SMN (Messico) per la tua regione.
...
graph TD
    ...
    C --> S["Sentinel — livello delle prove<br/>corrobora, pone il veto, alza il livello"]
    Y --> S
...
```mermaid
graph LR
    L["Compare il sottotitolo in diretta"] --> K["Audio grezzo riletto<br/>con tutto il contesto"] --> V["Confronto prudente<br/>(conserva ciò che è stato sentito)"] --> W["Riga corretta discretamente<br/>≤ 2 s, poi fissata"]
```
...
graph LR
    ...
    W --> N["Feed pubblici ufficiali<br/>NWS · MeteoGate · JMA · KMA · CMA<br/>ECCC · BOM · INMET · NDMA · SMN · USGS"]
```
