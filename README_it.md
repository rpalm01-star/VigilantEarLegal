...
### 🎛️ Oscilloscopio Acustico — vedi il suono come un ingegnere
...
- **Oscilloscopio Acustico** — visualizzazione del suono in tempo reale di livello professionale, gratis per tutti. (Gli strumenti di cattura per l'addestramento sono di Power Pack+.)
...
```mermaid
graph TD
    A["Tap del microfono stereo"] --> B["Istantanea del buffer condiviso"]
    B --> C["Classificatore di suoni (Apple Neural Engine)"]
    B --> D["TDOA / beamforming → direzione e distanza"]
    B --> E["Diarizzazione dei parlanti (WeSpeaker, ANE) → chi"]
    E --> F["Riconoscimento vocale (SpeechAnalyzer) → parole"]
    F --> G["Traduzione sul dispositivo → la tua lingua"]
    C & D & F & G --> H["Radar · sottotitoli · mappa · avvisi · Watch · Live Activity · AR"]
```
